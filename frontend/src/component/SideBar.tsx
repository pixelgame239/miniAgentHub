          ...
          navItems
            ...
            .map((item) => (
              <NavLink
                ...
                onClick={() => {
                    if(item.to === "/chat" && currentConversation) {
                      handleCreateNewChat(); // Nếu đang ở trang chat và có cuộc trò chuyện hiện tại, tạo mới
                    }
                    if(onClose) onClose();
                  }
                } // Đóng sidebar khi click vào link (chỉ có tác dụng trên mobile)
              >
                {item.icon}
